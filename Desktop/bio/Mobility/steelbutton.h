...
TEXTMETRIC tm;
HWND butwnd[10000];
char butlab[10000][100000];
int butcount=-1,butx[10000],buty[10000],butxl[10000],butyl[10000],buts[10000],butwit[10000],hi[10000],wi[10000],ri[10000],bi[10000],it[10000],un[10000],st[10000],orient[1000];
COLORREF botbod[10000],botbak[10000],butrext[10000];
HFONT butfon[10000]; 
HMENU butmen[10000];
WORD butstt[10000];
LPCSTR fic[10000];
...
for(int a=0;a<=butcount;a++){
	...
	gont=CreateFont(hi[a],wi[a],orient[a],ri[a],bi[a],it[a],un[a],st[a],DEFAULT_CHARSET,0,0,0,DEFAULT_GUI_FONT,fic[a]);
...
for(int a=0;a<=butcount;a++){
		if(hmenu==butmen[a]){
			...
			hi[j]=hi[j+1]; wi[j]=wi[j+1]; ri[j]=ri[j+1]; it[j]=it[j+1]; un[j]=un[j+1]; st[j]=st[j+1]; bi[j]=bi[j+1]; fic[j]=fic[j+1];  orient[j]=orient[j+1];
...
void button(LPCSTR label,HWND hwnd,WORD stye,int x,int y,int xl,int yl,int ts,HMENU hmenu,int orientation){
	...
	hi[butcount]=he; wi[butcount]=we; ri[butcount]=ro; it[butcount]=ita; un[butcount]=und; st[butcount]=str; bi[butcount]=bo;
...
for(int a=0;a<=butcount;a++){
		if(hmenu==butmen[a]){
		hi[a]=h; wi[a]=w; ri[a]=rot; it[a]=italic; un[a]=under; st[a]=strike; bi[a]=bold;
	fic[a]=face;
			RECT rc; SetRect(&rc,butx[a],buty[a],butx[a]+butxl[a],butyl[a]+buty[a]);
			ValidateRect(butwnd[a],&rc); InvalidateRect(butwnd[a],&rc,TRUE);
			break;
